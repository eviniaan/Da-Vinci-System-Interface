...
class XboxJointController(Node):
    ...
    def update_daVinci_commands(self):
        ...
        for i, index in enumerate(controller_indexes):
            controller_values = self.joystick_axes[index]
            self.joint_positions[i] += self.step_size * controller_values
    ...
    def update_kuka_commands(self):
        lin_x = self.joystick_axes[0]
        lin_y = self.joystick_axes[1]
        lin_z = self.joystick_axes[7]
        ang_z = self.joystick_axes[4]
            
        msg = Twist()
        msg.linear.x = 0.1 * lin_x
        msg.linear.y = 0.1 * lin_y
        msg.linear.z = 0.1 * lin_z
        msg.angular.x = 0.0
        msg.angular.y = 0.0
        msg.angular.z = 0.1 * ang_z
